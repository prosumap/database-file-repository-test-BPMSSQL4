...
-- =============================================
CREATE PROCEDURE [dbo].[spDailyUpd352ApdRtnByShipDate] 
...
--To replace qAppMkTbl_RtnByShipDate_2
...
SELECT CAT_DESCRIPTION, SUBCAT_DESCRIPTION, Ship_Date, Status, Line_Catalog, ROUND(SUM(Ship_Net$), 2) AS Ship_Net$, ROUND(SUM(Ship_Gross$), 2) AS Ship_Gross$, ROUND(SUM(Ship_Cost$), 2) AS Ship_Cost$, SUM(Ship_Units) 
                  AS Ship_Units, Ship_Fiscal_Wk, Ship_Fiscal_Month#, Ship_Fiscal_Month, Ship_Fiscal_Qtr, Ship_Fiscal_Year, Ship_Season, Ship_YYYYWK, Rtn_Date, ROUND(SUM(Rtn_Net$), 2) AS Rtn_Net$, ROUND(SUM(Rtn_Gross$), 2) AS Rtn_Gross$, 
                  ROUND(SUM(Rtn_Cost$), 2) AS Rtn_Cost$, SUM(Rtn_Units) AS Rtn_Units, Rtn_RsnCd, Rtn_RsnDesc, Rtn_Fiscal_Wk, Rtn_Fiscal_Month#, Rtn_Fiscal_Month, Rtn_Fiscal_Qtr, Rtn_Fiscal_Year, Rtn_Season, Rtn_YYYYWK, Exch_Date, 
                  ROUND(SUM(Exch_Net$), 2) AS Exch_Net$, ROUND(SUM(Exch_Gross$), 2) AS Exch_Gross$, ROUND(SUM(Exch_Cost$), 2) AS Exch_Cost$, SUM(Exch_Units) AS Exch_Units, Exch_RsnCd, Exch_RsnDesc, Exch_Fiscal_Wk, 
                  Exch_Fiscal_Month#, Exch_Fiscal_Month, Exch_Fiscal_Qtr, Exch_Fiscal_Year, Exch_Season, Exch_YYYYWK
FROM     vwDailyUpdSlct_RtnByShipDate_9
